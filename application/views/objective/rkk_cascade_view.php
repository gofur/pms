<?php
	...
	foreach ($Perspective as $row_1) {
		...
		echo '<thead><tr><th width="350px">Objective</th><th>Description</th><th>Action</th></tr></thead>';
		echo '<tbody>';
		foreach ($SO_List[$row_1->PerspectiveID] as $row_2) {
			...
			echo '<td>'.anchor('objective/rkk/edit_self_so/'.$row_2->SasaranStrategisID, 'Edit', 'class="fancybox-nonrefresh btn" data-fancybox-type="iframe"');'</td>';
			...
			echo '<thead><tr><th width="300">KPI</th><th>Count Type</th><th>YTD</th><th>Formula</th><th>Weight (%)</th><th>Target</th><th>Ref.</th><th witdh="80">Action</th></tr></thead>';
			echo '<tbody>';
			foreach ($KPI_List[$row_2->SasaranStrategisID] as $row_3){
				...
				echo '<thead><tr><th width="200">Subordinate</th><th>KPI</th><th width="250">Detail</th><th >Ref. Weight</th><th width="100">Action</th></tr></thead>';
				echo '<tbody>';
				foreach ($Cascade_List[$row_3->RKKDetailID] as $row_4) {
					...
					echo '<tr><td>Target</td><td>'.anchor($link['edit_target'].$row_4->RKKDetailID,thousand_separator($row_4->TargetAkhirTahun).' '.$row_4->Satuan,'class="fancybox" data-fancybox-type="iframe"').'</td></tr>';
						echo '</table>';
					echo '</td>';
					if ($row_3->ReferenceID ==3)
					{
						echo '<td>'.anchor($link['edit_ref_weight'].$row_4->RKKDetailID,$row_4->Ref_weight,'title="Edit Ref. Weight" class="fancybox" data-fancybox-type="iframe"').'</td>';	
					...
					else
					{
						...
						echo '<td>'.anchor($link['edit_kpi'].$row_4->RKKDetailID,'Edit KPI','class="btn fancybox" data-fancybox-type="iframe"' );
						echo '</td>';
					}
