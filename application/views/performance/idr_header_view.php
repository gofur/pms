<h3>Individual Development Realization</h3>
<?php 
	...
	<div class="span6">
		...
		<div class="row">
			...
			else
			{
				...
				echo form_open('performance/idr/',$attributes);
...
if(isset($subordinate)){
	...
	foreach ($subordinate as $row) {
		echo '<li>'.anchor($link['view_subordinateIDR'].$row->UserID.'/'.$row->PositionID.'/'.$Chief_RKKID,$row->NIK.' - '.$row->Fullname.' ('.$row->PositionName.')').'</li>';
	}
	echo '</ul></div></div>';	
}
?>
